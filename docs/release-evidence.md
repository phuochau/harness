...
| 8. Durable crash/recovery | `test/e2e/crash-matrix.test.ts`, `test/e2e/race-matrix.test.ts`, `test/integration/pi-process/controller-crash.test.ts`, and `test/integration/recover-cli.test.ts` cover fenced recovery, child survival, stopped-child continuation, exact process identity, and profile-backed CLI recovery. The real E2E recreates the runtime and accepts the completed Devin attempt from durable process/session evidence. |
...
| 10. Real disposable E2E | `npm run e2e:real` passed on 2026-09-21 in 168.48s: packed install, Codex Spec Kit specification/plan, Devin worktree implementation, runtime restart recovery, detached Codex review, exact fast-forward integration, and final tests. Full controller scheduling and crash matrices are covered deterministically by the non-subscription production E2E suite. |

## Release gates

| Gate | Result |
|---|---|
| `npm run verify` | Passed: 75 test files passed, 1 conditional file skipped; 328 tests passed, 4 opt-in/fixture tests skipped; clean package dry-run. |
| `npm run e2e:real` | Passed: 1 authenticated E2E test in 168.48s. |
...
| Review | Independent review of commit `d93fc12` found two critical and four important issues. All accepted findings are covered by the follow-up tests listed above; the corrected commit must receive a final no-blocker review before push. |
