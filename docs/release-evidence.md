...
| 10. Real disposable E2E | `npm run e2e:real` passed on 2026-09-21 in 237.58s: packed install, Codex Spec Kit specification/plan, Devin worktree implementation, signed process receipts, runtime restart recovery, detached Codex review, exact fast-forward integration, and final tests. Full controller scheduling and crash matrices are covered deterministically by the non-subscription production E2E suite. |

## Release gates

| Gate | Result |
|---|---|
| `npm run verify` | Passed: 78 test files passed, 1 conditional file skipped; 343 tests passed, 4 opt-in/fixture tests skipped; clean package dry-run. |
| `npm run e2e:real` | Passed: 1 authenticated E2E test in 237.58s. |
