  ...
  const workflow = compileWorkflow({
    ...
    workflow: {
      ...
      stages: [
        ...
        { id: "review", uses: "worker.review", runner: { prefer: ["codex", "claude", "devin"] }, needs: [{ stage: "implement", scope: "same-item" }], foreach: { source: "stages.tasks.outputs.graph", key: "task.id" }, policies: { require_different_worker_kind: true, scope: "task" }, retry: { max_attempts: 3, max_elapsed_seconds: 3600 }, on_failure: { changes_requested: { retry_stage: "implement" } } },
